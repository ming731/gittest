...
static int codeToOhm(s32* ohm, s32* Code, u16 *v_tdf, u16 *h_tdf)
{
	...
	ILI_DBG("douTVCH = %d, douTVCL = %d, douVariation = %d, douTDF1 = %d, douTDF2 = %d, douCint = %d, ratio = %d,douRinternal = %d\n"
	, douTVCH, douTVCL, douVariation, douTDF1, douTDF2, douCint, ratio, douRinternal);
	ILI_DBG("temp = ((douTVCH - douTVCL) * douVariation * (douTDF1 - douTDF2) * (1 << 12) / (9 * Code[j] * douCint)) * ratio;\n");
	ILI_DBG("temp = (temp - douRinternal) / 1000;\n");
