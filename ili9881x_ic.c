...
static struct ilitek_ic_func_ctrl func_ctrl[FUNC_CTRL_NUM] = {
	...
	[0] = {"sense", {0x1, 0x1, 0x0}, 3, 2},
	[1] = {"sleep", {0x1, 0x2, 0x0}, 3, 2},
	[2] = {"glove", {0x1, 0x6, 0x0}, 3, 2},
	[3] = {"stylus", {0x1, 0x7, 0x0}, 3, 2},
	[4] = {"tp_scan_mode", {0x1, 0x8, 0x0}, 3, 2},
	[5] = {"lpwg", {0x1, 0xA, 0x0}, 3, 2},
	[6] = {"gesture", {0x1, 0xB, 0x3F}, 3, 2},
	[7] = {"phone_cover", {0x1, 0xC, 0x0}, 3, 2},
	[8] = {"finger_sense", {0x1, 0xF, 0x0}, 3, 2},
	[9] = {"phone_cover_window", {0xE, 0x0, 0x0}, 3, 2},
	[10] = {"proximity", {0x1, 0x10, 0x0}, 3, 2},
	[11] = {"plug", {0x1, 0x11, 0x0}, 3, 0},
	[12] = {"edge_palm", {0x1, 0x12, 0x0}, 3, 0},
	[13] = {"lock_point", {0x1, 0x13, 0x0}, 3, 0},
	[14] = {"active", {0x1, 0x14, 0x0}, 3, 2},
	[15] = {"idle", {0x1, 0x19, 0x0}, 3, 2},
	[16] = {"gesture_demo_en", {0x1, 0x16, 0x0}, 3, 2},
	[17] = {"tp_recore", {0x1, 0x18, 0x0}, 3, 2},
	[18] = {"knock_en", {0x1, 0xA, 0x8, 0x03, 0x0, 0x0}, 6, 2},
	[19] = {"int_trigger", {0x1, 0x1B, 0x0}, 3, 2},
	[20] = {"ear_phone", {0x1, 0x17, 0x0}, 3, 0},
};
...
int ili_ic_func_ctrl(const char *name, int ctrl)
{
	...
	if (ret < 0)
		...
		if (func_ctrl[i].enble < 2 && (ctrl == ENABLE || ctrl == DISABLE))
			func_ctrl[i].enble = ctrl;

	ILI_DBG("set %s func to %d ", func_ctrl[i].name, func_ctrl[i].enble);
out:
	return ret;
}
...
void ili_ic_func_ctrl_reset(void) {
	...
	for (i = 0; i < FUNC_CTRL_NUM; i++) {
		if (func_ctrl[i].enble == ENABLE) {
			ILI_DBG("reset func ctrl %s\n", func_ctrl[i].name);
			if (ili_ic_func_ctrl(func_ctrl[i].name, ENABLE) < 0)
